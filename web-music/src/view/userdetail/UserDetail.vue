<template>
	<div id="user-detail" class="w">
		<div class="logined" v-if="$store.state.login.isLogin">
			<!-- 顶部基本信息 -->
			<div class="top-userinfo">
				...
				<div class="right-info">
					...
					<div class="row1">
						...
						<el-button
							size="small"
							v-if="uid != $store.state.login.userInfo.userId"
							class="subuser"
...
<script>
/* 网络请求 */
import { getUserDetail, getUserSonglist, getUserFollows, subUser } from "@/axiosReq/userdetail/userdetail";
...
export default {
	...
	computed: {
		...mapGetters("login",["userFollowsList", "userInfo","isLogin"]),
	},
	created() {
   ...
   */
		...
		getLoginUserFollows(page = 1) {
			...
			getUserFollows(this.userInfo.userId, offset, limit).then(res => {
				// 提交vuex保存登录用户的关注列表
				this.$store.dispatch("login/saveUserFollowsList", res.data.follow);
				this.isSubUser();
			});
		},
		// 判断是否关注了当前用户
